...
cmdsB = [
    ...
    ["kGetSMEnable", "?"],
    ["kSetSMEnable", "?"],
    ["kRecieveSMLogging", "".join(["fli" for x in range(3)])+"i?"],
    ["kGetSMLogEnable", "?"],
    ["kSetSMLogEnable", "?"],
    ["kStepSM", "ii"],
    ["kSetSMPosition", "lll"],
...
fnsB = {
    ...
    'kGetDemo': getTF,
    'kGetSMEnable': getTF,
    'kRecieveSMLogging': getLog,
    'kGetSMLogEnable': getTF,
    'kGetSMRemSteps': getEncoderCounts
}

#ardA = pcm.ArduinoBoard("/dev/ttyACM0", baud_rate=115200)
ardB = pcm.ArduinoBoard("/dev/ttyACM0", baud_rate=115200)
    ...
    while(True):
        ...
        if len(inpt) > 0:
            ...
            if inpt[0] == "pid":
                ...
                sndrcvBool(cB, "SMEnable", inpt)
            elif inpt[0] == "demo":
                sndrcvBool(cB, "Demo", inpt)
            elif inpt[0] == "pidlog":
                sndrcvBool(cB, "Logging", inpt)
            elif inpt[0] == "smlog":
                sndrcvBool(cB, "SMLogEnable", inpt)
            elif inpt[0] == "enc":
                ...
                if len(inpt) > 1:
                    cB.send("kSetEncoderCenter", int(inpt[1]))
                else:
                    cB.send("kSetEncoderCenter", -1)
            elif inpt[0] == "stp":
                if len(inpt) == 3:
                    cB.send("kStepSM", *[int(x) for x in inpt[1:]])
                else:
                    print(len(inpt))
            elif inpt[0] == "curpos":
                if (len(inpt) == 4):
                    cB.send("kSetSMPosition", *[int(x) for x in inpt[1:]])
            elif inpt[0] == "rmstps":
                cB.send("kGetSMRemSteps")
            elif inpt[0] == "tms":
                cB.send("kGetTimes")
